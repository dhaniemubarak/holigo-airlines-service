...
//        airlinesAvailabilityRepository.deleteAllAirlinesAvailabilityWhere();
    ...
    private AirlinesFinalFare getAirlinesFinalFare(long userId, Set<AirlinesFinalFareTrip> airlinesFinalFareTrips, TripType tripType, ResponseFareDto responseFareDto, Boolean isInternational) {
        BigDecimal ntaAmount = BigDecimal.valueOf(0.0);
        BigDecimal totalAmount = BigDecimal.valueOf(0.0);
        BigDecimal nraAmount;
        totalAmount = totalAmount.add(responseFareDto.getTotalAmount());
        ntaAmount = ntaAmount.add(responseFareDto.getNtaAmount());
        if (responseFareDto.getTotalAmountRet() != null) {
            totalAmount = totalAmount.add(responseFareDto.getTotalAmountRet());
            ntaAmount = ntaAmount.add(responseFareDto.getNtaAmountRet());
        }
        nraAmount = totalAmount.subtract(ntaAmount);
        FareDto fareDto = fareService.getFareDetail(FareDetailDto.builder()
                .ntaAmount(ntaAmount)
                .nraAmount(nraAmount)
                .productId(1).userId(userId).build());
...
//                .build();
//        for (AirlinesFinalFareTrip airlinesFinalFareTrip : airlinesFinalFareTrips) {
